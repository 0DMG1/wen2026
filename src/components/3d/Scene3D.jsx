...
const RAW_PLAYLIST = [
  ...
  { title: "Monsters", url: "/music/Monsters.m4a" },
  { title: "How do i say goodbye ", url: "/music/How_Do_I_Say_Goodbye.m4a" },
  { title: "Abrazame muy fuerte", url: "/music/Abrazame_muy_fuerte.mp3" },
  { title: "Te esperaba", url: "/music/Te_Esperaba.m4a" },
  { title: "Gracias a ti", url: "/music/Gracias_a_Ti.m4a" },
  { title: "Fascinacion", url: "/music/Fascinacion.mp3" },
  { title: "Favorito", url: "/music/Favorito.m4a" },
  { title: "Amor de mujer", url: "/music/Amor_de_Mujer.m4a" },
  { title: "Solo el cielo y tu", url: "/music/Solo_el_Cielo_y_Tu.mp3" },
  { title: "El poder de tu amor", url: "/music/El_Poder_de_Tu_Amor.m4a" },
  { title: "La cima del cielo", url: "/music/La_Cima_del_Cielo.mp3" },
  { title: "Solamente tu", url: "/music/Solamente_Tu.mp3" },
...
const BIRTHDAY_MESSAGES = [
  "✨ MIRA HACIA LAS ESTRELLAS, WENDY ✨",
  "¡FELIZ CUMPLEAÑOS IRIS MI AMOR! 🎂",
  "¡15 AÑOS NAVEGANDO JUNTOS! ⚓",
  "¡♒ ACUARIANA MÍSTICA ♒!",
  "¡ERES MI SUEÑO EN LA REALIDAD! 💫",
  "TU RISA ES MI ÚNICO FARO 🏮",
  "¡FELIZ DÍA MI ESPOSITA HERMOSA! ❤️",
  "MAMA DE BRUCE Y GOLIATH  🐾"
];

const RIPPLE_MESSAGES = [
  "Tu inteligencia es la fosa más profunda y tu humor ácido es el que nos salva siempre.",
  "Eres mi puerto seguro en cualquier tormenta. ¡Te amo, mi guerrera!",
  "Gracias por 15 años de navegar juntos. Eres el faro que guía mi vida.",
  "¡Feliz cumpleaños, reina de mi océano! Que la marea siempre esté a tu favor.",
  "Ni miles amaneceres bastan para decirte lo mucho que me encantas, amor de mi vida.",
  "Las estrellas te cuidan, y yo te sostengo aquí en nuestra orilla.",
  "Goliath y Bruce dicen que eres la mejor mamá del mundo (y yo también).",
  "Eres el regalo más hermoso que encontré en mi vida.",
  "Me encantas en todas tus facetas: guerrera, inteligente y mi berrinchuda favorita.",
  "Cada día que pasa, me enamoro más de tus ojos, de tu corazón y tu alma de Acuario.",
  "Gracias por ser mi pilar en el 2025 y por nadar siempre a mi lado.",
  "¡Feliz cumple, mami! Hoy todo el santuario brilla solo para ti.",
  "Tu risa es el sonido más bello de este océano. Nunca dejes de brillar.",
  "15 años después, sigo agradeciendo al destino por aquel proyecto final en UPEM."
];

const ESFERA_POOL = [
  "/photos/esferas/00.webp", "/photos/esferas/01.webp", "/photos/esferas/02.webp", 
  "/photos/esferas/03.webp", "/photos/esferas/04.webp", "/photos/esferas/05.webp", 
  "/photos/esferas/06.webp", "/photos/esferas/07.webp", "/photos/esferas/08.webp",
  "/photos/esferas/09.webp", "/photos/esferas/10.webp", "/photos/esferas/11.webp",
  "/photos/esferas/12.webp", "/photos/esferas/13.webp", "/photos/esferas/14.webp",
  "/photos/esferas/15.webp", "/photos/esferas/16.webp", "/photos/esferas/17.webp",
  "/photos/esferas/18.webp", "/photos/esferas/19.webp", "/photos/esferas/20.webp",
  "/photos/esferas/21.webp", "/photos/esferas/22.webp", "/photos/esferas/23.webp"
];
...
const StellarGuideBeam = ({ active }) => {
  ...
  useFrame(({ clock }) => { if (beamRef.current) { beamRef.current.rotation.y = clock.getElapsedTime() * 0.5; beamRef.current.material.opacity = active ? 0.3 + Math.sin(clock.getElapsedTime() * 2) * 0.1 : 0; } });
  return (<mesh ref={beamRef} position={[0, 10, -45]} rotation={[0, 0, 0]}><cylinderGeometry args={[8, 15, 120, 32, 1, true]} /><meshStandardMaterial color="#ffd700" emissive="#ffaa00" emissiveIntensity={2} transparent side={THREE.DoubleSide} depthWrite={false} /></mesh>);
};
const FloatingLanterns = ({ isVisible }) => {
  const count = 25; const lanternData = useMemo(() => Array.from({ length: count }).map(() => ({ pos: [(Math.random() - 0.5) * 110, -45, (Math.random() - 0.5) * 80], speed: 0.03 + Math.random() * 0.06, scale: 0.8 + Math.random() * 0.5, flicker: Math.random() * 10 })), []); const groupRef = useRef();
  useFrame(({ clock }) => { const t = clock.getElapsedTime(); if (!groupRef.current) return; groupRef.current.children.forEach((instance, i) => { const d = lanternData[i]; if (isVisible) { instance.position.y = -35 + (t * d.speed * 8) % 90; instance.position.x += Math.sin(t * 0.5 + i) * 0.01; instance.scale.setScalar(d.scale); if (instance.children[1]) instance.children[1].intensity = 1.5 + Math.sin(t * 4 + d.flicker) * 0.5; } else { instance.scale.setScalar(0); } }); });
  return (<group ref={groupRef}>{lanternData.map((d, i) => (<group key={i} position={d.pos}><mesh><cylinderGeometry args={[0.5, 0.45, 1.2, 8]} /><meshStandardMaterial color="#ffcc66" emissive="#ffaa33" emissiveIntensity={0.5} transparent opacity={0.6} /></mesh><pointLight color="#ff8800" intensity={2} distance={15} /><mesh position={[0,-0.2,0]}><sphereGeometry args={[0.15, 8, 8]} /><meshBasicMaterial color="#fff" /></mesh></group>))}</group>);
};
const TransitionFlash = ({ active }) => { const [show, setShow] = useState(false); useEffect(() => { setShow(true); const t = setTimeout(() => setShow(false), 800); return () => clearTimeout(t); }, [active]); return <Html fullscreen style={{ pointerEvents: 'none' }}><div className={`w-full h-full bg-white transition-opacity duration-700 ${show ? 'opacity-30' : 'opacity-0'}`} /></Html>; };
...
const GiantFloatingBanner = ({ isBirthdayMode }) => { const bannerRef = useRef(); const [msgIdx, setMsgIdx] = useState(0); const xPos = useRef(250); const speed = 25; useEffect(() => { if (isBirthdayMode) { setMsgIdx(0); xPos.current = 150; } }, [isBirthdayMode]); useFrame((state, delta) => { xPos.current -= speed * delta; if (xPos.current < -250) { xPos.current = 250; setMsgIdx((prev) => (prev + 1) % BIRTHDAY_MESSAGES.length); } if (bannerRef.current) { bannerRef.current.position.set(xPos.current, 20 + Math.sin(state.clock.getElapsedTime() * 1.5) * 8, 40); } }); return (<group ref={bannerRef}><Html center distanceFactor={15}><div className={`font-['Cinzel'] font-bold text-[10rem] md:text-[25rem] whitespace-nowrap select-none italic uppercase px-10 ${isBirthdayMode ? 'banner-shimmer-gold' : 'banner-shimmer-cyan'}`}>{BIRTHDAY_MESSAGES[msgIdx]}</div></Html></group>); };
...
const DynamicPhotoSlot = ({ position, initialIndex, delay, onSelect, pool }) => {
  const [photoIndex, setPhotoIndex] = useState(initialIndex);
  // Seguridad extra: si el pool está vacío al inicio, no hace nada hasta que se llene
  useEffect(() => {
    if (!pool || pool.length === 0) return;
    const interval = setInterval(() => {
      setPhotoIndex((p) => (p + 1) % pool.length);
    }, 15000 + (delay * 1000));
    return () => clearInterval(interval);
  }, [delay, pool]);

  // Usa la primera imagen como fallback si el índice aún no está listo
  const imageToLoad = (pool && pool.length > 0) ? pool[photoIndex] : ESFERA_POOL[0];
  const tex = useLoader(THREE.TextureLoader, imageToLoad);

  return (<Float speed={2}><group position={position} onPointerDown={(e) => { e.stopPropagation(); onSelect(imageToLoad); }}><mesh><sphereGeometry args={[4, 64, 64]} /><MeshTransmissionMaterial thickness={0.5} transmission={1} color="#aeeeee" /></mesh><mesh position={[0,0,0.1]}><circleGeometry args={[3.5, 32]} /><meshBasicMaterial map={tex} side={THREE.DoubleSide} transparent opacity={0.9} /></mesh></group></Float>);
};
...
const OceanContent = ({ isLowTide, setFocusedPhoto, isPlaying }) => {
  // 1. Barajamos todas las imágenes una sola vez al cargar el componente
  const bigShuffledPool = useMemo(() => shuffleArray(ESFERA_POOL), []);

  // 2. Repartimos las imágenes en sub-pools únicos para cada esfera
  const distributedPools = useMemo(() => {
    const chunkSize = Math.ceil(bigShuffledPool.length / SPHERE_SLOTS.length); // Aprox 5 imágenes por esfera
    return SPHERE_SLOTS.map((_, i) => {
      const start = i * chunkSize;
      const end = start + chunkSize;
      return bigShuffledPool.slice(start, end); // Cada esfera recibe su propio trozo único
    });
  }, [bigShuffledPool]);
  ...
  return (
    <>
      ...
      <Suspense fallback={null}> 
        ...
        {/* 3. Renderizamos cada esfera pasándole SU propio pool único */}
        {SPHERE_SLOTS.map((s, i) => (
          <DynamicPhotoSlot 
            key={`s-${i}`} 
            position={s.pos} 
            initialIndex={0} // Siempre empieza en la primera foto de su montoncito
            delay={s.delay} 
            onSelect={setFocusedPhoto} 
            pool={distributedPools[i]} // <-- Aquí está la clave: cada esfera recibe un pool distinto
          />
        ))}
...
export const Scene3D = ({ isLowTide, setIsLowTide, setFocusedPhoto, focusedPhoto, showLetter }) => {
  ...
  const amaneceres = useMemo(() => { const fechaInicio = new Date('2010-04-23'); const fechaHoy = new Date(); const diferencia = fechaHoy - fechaInicio; return Math.floor(diferencia / (1000 * 60 * 60 * 24)); }, []);
  const handleRippleClick = (e) => { e.stopPropagation(); const randomMsg = RIPPLE_MESSAGES[Math.floor(Math.random() * RIPPLE_MESSAGES.length)]; alert(randomMsg); };

  return (
    <div className="relative w-full h-full overflow-hidden bg-[#001e36]">
      <style>{`@keyframes shimmer-move { 0% { background-position: -200% center; } 100% { background-position: 200% center; } } .banner-shimmer-cyan { background: linear-gradient(90deg, rgba(255,255,255,0.1) 20%, rgba(34,211,238,0.9) 50%, rgba(255,255,255,0.1) 80%); background-size: 200% auto; -webkit-background-clip: text; -webkit-text-fill-color: transparent; animation: shimmer-move 4s infinite linear; filter: drop-shadow(0 0 15px rgba(34,211,238,0.4)); } .banner-shimmer-gold { background: linear-gradient(90deg, rgba(251,191,36,0.3) 20%, rgba(255,255,255,1) 50%, rgba(251,191,36,0.3) 80%); background-size: 200% auto; -webkit-background-clip: text; -webkit-text-fill-color: transparent; animation: shimmer-move 3s infinite linear; filter: drop-shadow(0 0 20px rgba(251,191,36,0.6)); }`}</style>
      <div className={`absolute inset-0 pointer-events-none z-50 flex flex-col justify-between transition-all duration-700 ${showLetter || focusedPhoto ? 'opacity-0' : 'opacity-100'}`}>
        <div className="p-3 md:p-10 flex flex-row justify-between items-start gap-2">
          <div className="flex flex-col gap-2 pointer-events-auto max-w-[65%]">
            <div className={`transition-all duration-1000 p-4 md:p-7 rounded-[1.8rem] border shadow-2xl scale-95 md:scale-100 origin-top-left ${isLowTide ? 'bg-amber-950/70 border-amber-500/50 shadow-amber-500/20' : 'bg-black/70 border-white/20'}`}>
              ...
              <div className="flex flex-col gap-4 mt-4 border-t border-white/10 pt-3 text-center">
                  <span className={`text-[11px] font-['Montserrat'] uppercase font-bold tracking-widest truncate max-w-[140px] md:max-w-[200px] mx-auto transition-all duration-500 ${isPlaying ? (isLowTide ? 'text-amber-300 drop-shadow-[0_0_8px_gold]' : 'text-cyan-300 drop-shadow-[0_0_8px_cyan]') : 'text-white/40 italic'}`}>{isPlaying ? `-> ${shuffledPlaylist[trackIndex].title}` : `Pausado`}</span>
                  <div className="flex items-center justify-center gap-4 md:gap-6">
                    ...
                    <button onClick={togglePlay} className={`w-14 h-14 md:w-16 md:h-16 rounded-full flex items-center justify-center transition-all duration-500 border-2 ${isPlaying ? (isLowTide ? 'bg-amber-500/30 border-amber-400 scale-110 shadow-[0_0_15px_rgba(255,191,0,0.4)]' : 'bg-cyan-500/30 border-cyan-400 scale-110 shadow-[0_0_15px_rgba(34,211,238,0.4)]') : 'bg-white/10 border-white/30'}`}>{isPlaying ? <FiPause className={`${isLowTide ? 'text-amber-300' : 'text-cyan-300'} text-2xl md:text-3xl`} /> : <FiPlay className="text-white text-2xl md:text-3xl ml-1" />}</button>
          ...
          <div className={`pointer-events-auto transition-all duration-1000 p-4 md:p-7 rounded-[1.8rem] border shadow-2xl text-right scale-95 md:scale-100 origin-top-right ${isLowTide ? 'bg-amber-950/70 border-amber-500/50 shadow-amber-500/20' : 'bg-black/70 border-white/20'}`}>
              ...
              <span className="text-white text-4xl md:text-7xl font-['Montserrat'] font-extrabold leading-none">{amaneceres.toLocaleString()}</span>
          </div>
        </div>
        <div className="p-6 md:p-12 flex justify-between items-end">
          <div className="pointer-events-auto"><button onClick={() => setIsLowTide(!isLowTide)} className={`w-24 h-24 rounded-full border-4 flex items-center justify-center transition-all duration-1000 shadow-2xl ${isLowTide ? 'bg-amber-500/80 border-amber-200 scale-110 shadow-amber-500/50' : 'bg-black/40 border-cyan-500/50 hover:bg-cyan-500/20'}`}><TbZodiacAquarius className={`text-5xl transition-all duration-700 ${isLowTide ? 'text-amber-100 rotate-180 drop-shadow-[0_0_20px_gold]' : 'text-cyan-400 drop-shadow-[0_0_10px_cyan]'}`} /></button></div>
          <div className="flex flex-col items-end gap-10 pointer-events-auto pb-24 md:pb-28"><button onClick={handleRippleClick} className={`w-16 h-16 rounded-full border-2 flex items-center justify-center shadow-lg hover:scale-110 group transition-all duration-1000 ${isLowTide ? 'bg-amber-600/40 border-amber-400/50' : 'bg-cyan-600/40 border-white/30'}`}><TbRipple className={`text-4xl transition-colors duration-1000 ${isLowTide ? 'text-amber-200' : 'text-cyan-300'} group-hover:text-white group-hover:drop-shadow-[0_0_15px_white]`} /></button></div>
        </div>
      </div>
      <div className="absolute inset-0 z-0"><Canvas camera={{ position: [0, 0, 75], fov: 45 }} gl={{ antialias: true, alpha: true }}><Suspense fallback={null}><OceanContent isLowTide={isLowTide} setFocusedPhoto={setFocusedPhoto} isPlaying={isPlaying} /></Suspense></Canvas></div>
    </div>
  );
};