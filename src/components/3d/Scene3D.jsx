...
// ... (RAW_PLAYLIST, MESSAGES y POOLS se mantienen igual)
const RAW_PLAYLIST = [
...
const BIRTHDAY_MESSAGES = [ "¡FELIZ CUMPLEAÑOS IRIS, AMOR MIO! 🎂", "¡ERES MI TODO! 🌹", "¡ERES MI SUEÑO REALIDAD! 💫" ];
...
const GiantFloatingBanner = () => { const bannerRef = useRef(); const [msgIdx, setMsgIdx] = useState(0); const xPos = useRef(250); const speed = 25; useFrame((state, delta) => { xPos.current -= speed * delta; if (xPos.current < -250) { xPos.current = 250; setMsgIdx((prev) => (prev + 1) % BIRTHDAY_MESSAGES.length); } if (bannerRef.current) { const yShift = Math.sin(state.clock.getElapsedTime() * 1.5) * 8; bannerRef.current.position.set(xPos.current, 20 + yShift, 40); } }); return ( <group ref={bannerRef}><Html center distanceFactor={15}><div className="font-['Cinzel'] font-bold text-white text-[10rem] md:text-[25rem] whitespace-nowrap drop-shadow-[0_0_60px_rgba(255,255,255,1)] italic uppercase px-10">{BIRTHDAY_MESSAGES[msgIdx]}</div></Html></group> ); };
const MemorialStar = ({ position, name, color, isLowTide, audioUrl, isPlaying }) => { const flareRef = useRef(); const starTex = useMemo(() => createSuperFlareTexture(), []); useFrame(({ clock }) => { const t = clock.getElapsedTime(); const pulse = (isLowTide ? 6 : 4) + Math.sin(t * 3) * 0.8; if (flareRef.current) { flareRef.current.scale.set(pulse * 6, pulse * 6, 1); flareRef.current.rotation.z = t * 0.1; } }); return ( <group position={position}><sprite ref={flareRef}><spriteMaterial map={starTex} color={color} transparent blending={THREE.AdditiveBlending} opacity={1} /></sprite><pointLight intensity={isLowTide ? 600 : 250} distance={100} color={color} decay={1.5} />{isPlaying && audioUrl && (<Suspense fallback={null}><PositionalAudio url={audioUrl} distance={25} loop /></Suspense>)}<Html position={[0, -4, 0]} center><div className="font-['Cinzel'] text-3xl md:text-5xl text-white opacity-90 whitespace-nowrap drop-shadow-[0_0_15px_rgba(255,255,255,0.7)]">{name}</div></Html></group> ); };
...
const OceanContent = ({ isLowTide, setFocusedPhoto, isPlaying }) => { const treasureData = useMemo(() => { const shuffled = shuffleArray(COUPON_POOL); return shuffled.map((url, i) => { const isBottle = i < 6; const pos = isBottle ? [(Math.random() - 0.5) * 85, (Math.random() - 0.5) * 30, (Math.random() * -40) - 10] : [[-45, 0, 45, -35, 35, 0][i-6], -28, [-55, -60, -55, -30, -30, -40][i-6]]; return { url, pos, type: isBottle ? 'bottle' : 'chest', delay: Math.random() * 5 }; }); }, []); const color = isLowTide ? '#2a1a00' : '#001e36'; return ( <> <CameraListener /> <color attach="background" args={[color]} /> <fog attach="fog" args={[color, 10, 150]} /> <ambientLight intensity={1.5} /> <GiantFloatingBanner /> <group position={[0, 45, -45]}> <MemorialStar position={[-35, 5, 0]} name="." color="#50ffb1" isLowTide={isLowTide} audioUrl="/music/How_Do_I_Say_Goodbye.m4a" isPlaying={isPlaying} /> <MemorialStar position={[35, 12, -10]} name="." color="#00e5ff" isLowTide={isLowTide} audioUrl="/music/Monsters.m4a" isPlaying={isPlaying} /> <MemorialStar position={[15, 20, -30]} name="." color="#ffffff" isLowTide={isLowTide} audioUrl="/music/Abrazame_muy_fuerte.mp3" isPlaying={isPlaying} /> </group> <Suspense fallback={null}> <Sparkles count={600} scale={110} size={35} speed={0.7} /> <WalkingPath type="bulldog" startPos={[50, 0, -60]} endPos={[-40, 0, 20]} count={22} pathDelay={0} /> <WalkingPath type="small" startPos={[-45, 0, -50]} endPos={[35, 0, 10]} count={30} pathDelay={4} /> <WalkingPath type="tiny" startPos={[20, 0, -65]} endPos={[-20, 0, 15]} count={40} pathDelay={8} /> {SPHERE_SLOTS.map((s, i) => <DynamicPhotoSlot key={`s-${i}`} position={s.pos} initialIndex={i % ESFERA_POOL.length} delay={s.delay} onSelect={setFocusedPhoto} />)} {treasureData.map((item, i) => item.type === 'bottle' ? <MessageBottle key={`b-${i}`} position={item.pos} url={item.url} delay={item.delay} onSelect={setFocusedPhoto} /> : <TreasureChest key={`c-${i}`} position={item.pos} url={item.url} onSelect={setFocusedPhoto} />)} </Suspense> <OrbitControls enablePan={false} autoRotate autoRotateSpeed={0.3} maxDistance={95} minDistance={30} /> <Environment preset="night" /> </> ); };

// EXPORTACIÓN NOMBRADA CLAVE
      ...
      <div className={`absolute inset-0 pointer-events-none z-50 flex flex-col justify-between transition-all duration-700 ${showLetter || focusedPhoto ? 'opacity-0' : 'opacity-100'}`}>
        ...
        <div className="p-3 md:p-10 flex flex-row justify-between items-start gap-2">
          <div className="flex flex-col gap-2 pointer-events-auto max-w-[65%]">
            <div className="bg-black/70 backdrop-blur-3xl p-4 md:p-7 rounded-[1.8rem] border border-white/20 shadow-2xl scale-95 md:scale-100 origin-top-left">
              <h1 className="text-white text-3xl md:text-5xl font-['Cinzel'] tracking-[0.2em] leading-none text-center">Iris Wendy</h1>
              <p className="text-cyan-300 text-[10px] md:text-[11px] font-['Montserrat'] tracking-[0.3em] mt-2 uppercase font-black text-center">Refugio 2026</p>
              <div className="flex flex-col gap-4 mt-4 border-t border-white/10 pt-3 text-center">
                  <span className={`text-[11px] font-['Montserrat'] uppercase font-bold tracking-widest truncate max-w-[140px] md:max-w-[200px] mx-auto transition-all duration-500 ${isPlaying ? 'text-cyan-300 drop-shadow-[0_0_8px_rgba(34,211,238,0.8)]' : 'text-white/40 italic'}`}>
                      {isPlaying ? `-> ${shuffledPlaylist[trackIndex].title}` : `Pausado: ${shuffledPlaylist[trackIndex].title}`}
                  </span>
                  <div className="flex items-center justify-center gap-4 md:gap-6">
                    <button onClick={handlePrev} className="text-white/40 hover:text-cyan-400 text-2xl transition-all p-2 hover:drop-shadow-[0_0_8px_cyan]"><FiSkipBack /></button>
                    <button onClick={togglePlay} className={`w-14 h-14 md:w-16 md:h-16 rounded-full flex items-center justify-center transition-all duration-500 border-2 ${isPlaying ? 'bg-cyan-500/30 border-cyan-400 scale-110 shadow-[0_0_20px_rgba(34,211,238,0.6)]' : 'bg-white/10 border-white/30 hover:bg-white/20'}`}>
                      {isPlaying ? <FiPause className="text-cyan-300 text-2xl md:text-3xl drop-shadow-[0_0_10px_rgba(34,211,238,1)]" /> : <FiPlay className="text-white text-2xl md:text-3xl ml-1" />}
                    </button>
                    <button onClick={handleNext} className="text-white/40 hover:text-cyan-400 text-2xl transition-all p-2 hover:drop-shadow-[0_0_8px_cyan]"><FiSkipForward /></button>
                  </div>
              </div>
            </div>
          </div>
          <div className="pointer-events-auto bg-black/70 backdrop-blur-3xl p-4 md:p-7 rounded-[1.8rem] border border-white/20 shadow-2xl text-right scale-95 md:scale-100 origin-top-right">
              <span className="text-cyan-300 text-[10px] font-['Montserrat'] tracking-[0.2em] block uppercase font-black mb-1">Amaneceres</span>
        ...
        <div className="p-6 md:p-12 flex justify-between items-end">
          <div className="pointer-events-auto">
             <button onClick={() => setIsLowTide(!isLowTide)} className={`w-24 h-24 rounded-full border-4 flex items-center justify-center transition-all duration-1000 shadow-2xl ${isLowTide ? 'bg-amber-400/80 border-white scale-110 shadow-amber-500/50' : 'bg-black/40 border-cyan-500/50 hover:bg-cyan-500/20'}`}>
               <TbZodiacAquarius className={`text-5xl transition-all ${isLowTide ? 'text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.8)]' : 'text-cyan-400 drop-shadow-[0_0_10px_cyan]'}`} />
             </button>
          </div>
          <div className="flex flex-col items-end gap-10 pointer-events-auto pb-24 md:pb-28">
            <button 
              ...
              className="w-16 h-16 rounded-full bg-cyan-600/40 border-2 border-white/30 flex items-center justify-center shadow-lg hover:scale-110 group transition-all"
            >
              <TbRipple className="text-4xl text-cyan-300 drop-shadow-[0_0_10px_rgba(34,211,238,0.6)] group-hover:text-white group-hover:drop-shadow-[0_0_15px_white] transition-all" />
            </button>
          </div>
        </div>
      </div>
